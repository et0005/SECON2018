...
def range_check_chest(tolerance_max, tolerance_min):
    
    left_value = int(ultrasonic.sensorL.sonic_read())
    right_value = int(ultrasonic.sensorR.sonic_read())
    
    if left_value >= tolerance_max or right_value >= tolerance_max:
        # totally out of range
        return 3
    else:
        left_min = left_value - 4  # define range value for sensors
...
def adjust_chest(tolerance_max, tolerance_min):
...
def backtrack_a(InfraredSensor):
    ...
    if IR_control.IR1.A == 0:  # turn towards plank
...
def backtrack_b(InfraredSensor):
    ...
    adjust_chest(tolerance_max['chest'], tolerance_min['chest'])
